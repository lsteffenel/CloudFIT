 ...
 */
public class JobsSchedulerOld extends Thread implements Serializable {
    ...
    public void addJob(JobMessage obj, ServiceInterface comm) {
        ...
        if (obj.getJar() == null) { // regular submission without a jar
            ...
            JobManagerInterface TS = TheBigFactory.getJobManager(comm, jobId, jobClass, jobargs, reqs);
            if (obj.getData() != null) {
            ...
            try {
                ...
                Logger.getLogger(JobsSchedulerOld.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void run() {
        while (true) {
            ...
            while (it.hasNext()) {
                JobManagerInterface element = it.next();
                if (element.getStatus() == element.NEW || element.getStatus() == element.NOMATCH) {
