...
// Exported constructors.
     ...
     * <TT>jar</TT> is null.
     * @exception IOException Thrown if the JAR file's contents could not be
     * extracted from the
     * <TT>jar</TT> byte array.
     */
    public JarLoader(byte[] jar)
            throws IOException {
        super();
        readJarContents(jar);
    }
    ...
    private void readJarContents(byte[] jar)
            throws IOException {
        if (jar == null) {
...
// Hidden operations.
    ...
    protected Class<?> findClass(String name)
        ...
        if (content == null) {
            throw new ClassNotFoundException(String.format("JarClassLoader.findClass(): Class %s not found", name));
        }
