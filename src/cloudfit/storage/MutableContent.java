 ...
 */
public class MutableContent extends ContentHashPastContent implements Serializable, DHTContent {
     ...
     */
    public MutableContent(Id id, Serializable content) {
        super(id);
...
//
            if (!(((MutableContent) existingContent).getContent() instanceof DHTStorageUnit)) {
                throw new PastException(
                        "MutableContent : can't insert, existing object for the TID("
                        + this.myId + ") is of unknown class type" + ((MutableContent) existingContent).getContent().getClass());
            } else {
                synchronized (existingContent) {
                    // Update existing Catalog entry
                    MutableContent ctt = (MutableContent) existingContent;
